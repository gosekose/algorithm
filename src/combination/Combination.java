...
public class Combination {
    ...
    List<List<Integer>> getCombination() {
        ...
        getCombinationHelper(arr, 3, 0, new ArrayList<>());

        return combi;
    }

    void getCombinationHelper(List<Integer> input, int k, int start, List<Integer> current) {
        if (k == 0) {
        ...
        for (int i = start; i < input.size(); i++) {
            ...
            getCombinationHelper(input, k - 1, i + 1, current);
...
class CombinationTest {
    public static void main(String[] args) {
        ...
        System.out.println(combi.get(0).get(0));
        System.out.println(combi.get(0).get(1));
        System.out.println(combi.get(0).get(2));
    }
}
